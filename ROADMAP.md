...
- Get: an API key from any provider in [PROVIDERS.md](./PROVIDERS.md), with a spend cap set. Groq and Google have free tiers; Ollama needs no key at all
...
- **Deliverable:** Screenshot of a successful `chat.completions.create` response

> 📎 **Read this week or next — [Module 16: Working with AI Coding Agents](./modules/16-ai-coding-agents.md).** The meta-skill of the 2026 job market: driving Claude Code / Cursor / Copilot-class tools with a real workflow — context files, right-sized task specs, a review protocol for AI-written diffs, verification loops, and guardrails. Every weekend project from here on goes faster if you set this up now, and "how do you work with coding agents?" is now a standard interview question.
...
- Read: the [Chat Completions API reference](https://platform.openai.com/docs/api-reference/chat) — the shape every compatible provider implements
- Video: [Prompt Engineering Overview (30m)](https://www.youtube.com/watch?v=T9aRN5JkmL8)
...
- Tools: Zod + a forced tool call — the portable structured-output pattern (`response_format` is unevenly honoured; see [PROVIDERS.md](./PROVIDERS.md))
- Read: [Function calling guide](https://platform.openai.com/docs/guides/function-calling)
...
- Read: your provider's function-calling page — note what it does *not* support
...
- Read in full: [Anthropic's Prompt Engineering Guide](https://docs.claude.com/en/docs/build-with-claude/prompt-engineering/overview) — one vendor's, mostly general craft
- Do every exercise in: [the Prompt Engineering Interactive Tutorial](https://github.com/anthropics/prompt-eng-interactive-tutorial), pointed at your own provider
...
- Read: [published production system prompts](https://docs.claude.com/en/release-notes/system-prompts) — real ones, worth reading closely whichever model you use
- Compare with a system prompt collection for your own model family
...
- **Project:** Build an MCP server exposing your local filesystem + a custom tool. Use it from any MCP client (Claude Desktop, Cursor, Windsurf, Zed, VS Code).
...
- Read: your provider's vision docs — confirm your model accepts images at all
- Read: [Computer use — one vendor's implementation](https://www.anthropic.com/news/3-5-models-and-computer-use), for the architecture
...
- Read: your AI coding agent's docs on test generation
...
- Read: your provider's prompt-caching docs — a provider-native feature worth taking deliberately
...
- Video: [AI Red Teaming (research talk, 40m)](https://www.youtube.com/watch?v=-vK8WpDXFLI)
...
### 🟥 Week 25 — n8n Fundamentals + LLM Integration
**Goal:** Self-host n8n, call an LLM from a workflow, ship a Slack → AI → Slack bot.
...
- **Project:** A Slack bot that classifies QA alerts with your cheap model and posts verdicts back to the thread, with an error-handling workflow.

### 🟥 Week 26 — n8n + Your QA Stack
**Goal:** Wire n8n to the tools you built in Weeks 1–24 so non-engineers can trigger them.
- **Project:** A CI failure triage pipeline (GitHub → LLM → Jira + Slack), an MCP-tools HTTP wrapper callable from a webhook, and a nightly AI quality digest.
